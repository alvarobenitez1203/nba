...
<html>
    <head>
        <meta charset="UTF-8">
        <title>NBA | Modificar equipo</title>
    ...
    <body>
        ...
        <h1 class="text-center mb-5 bg-primary text-white p-5">MODIFICAR EQUIPO</h1>
        ...
        <?php 
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['modificar']) && !empty($_POST['modificar'])) {
                ...
                $url_dame_comprobarequipo = 'http://iescristobaldemonroy.duckdns.org:81/USER10/NBA_Microservicios/services/comprobarNombreEquipo.php?nombre=' . $nombre . '&idE=' . $idE;
                ...
                if ($xml->status == 'OK') {
                        ...
                        if ($_FILES['logoEquipo']['size'] > 0) {
                            $imagenNuevaNombre = urldecode($nombre) . '.png';
                            $imagenNuevaTmpName = $_FILES['logoEquipo']['tmp_name'];
                            ...
                            if (getimagesize($imagenNuevaTmpName)[0] == getimagesize($imagenNuevaTmpName)[1]) {
                                ...
                                if (strtolower($extensionValida) == 'png') {
                                    move_uploaded_file($imagenNuevaTmpName, 'img/equipos/' . $imagenNuevaNombre);
                    
                                    if ($nombre != $equipoAnterior) {
                                        $imagenAnterior = 'img/equipos/' . $equipoAnterior . '.png';
                                        if (file_exists($imagenAnterior)) {
                                            unlink($imagenAnterior);
                                        }
                                    }
                    
                                    $url_dame_modificarequipo = 'http://iescristobaldemonroy.duckdns.org:81/USER10/NBA_Microservicios/services/modificarEquipos.php?idequipo='. $idE.'&nombre='. $nombre . '&pais=' . $pais . '&annofundacion=' .$annofundacion. '&conferencia=' . $conferencia;

                                    // Hacer la solicitud HTTP y obtener el XML como una cadena
                                    $xmlString = file_get_contents($url_dame_modificarequipo);
                            ...
                            $url_dame_modificarequipo = 'http://iescristobaldemonroy.duckdns.org:81/USER10/NBA_Microservicios/services/modificarEquipo.php?idequipo='. $idE.'&nombre='. $nombre . '&pais=' . $pais . '&annofundacion=' .$annofundacion. '&conferencia=' . $conferencia;
                            ...
                            if ($nombre != $equipoAnterior) {
                                $imagenAnterior = 'img/equipos/' . $equipoAnterior . '.png';
                                $imagenNueva = 'img/equipos/' . $nombre . '.png';
            ...
            $url_dame_datos = 'http://iescristobaldemonroy.duckdns.org:81/USER10/NBA_Microservicios/services/datosEquipo.php?idequipo='.$idEquipo;
            ...
            foreach ($xml->equipo as $equipo) {
                $nombre = urldecode($equipo->nombre);
                $pais = urldecode($equipo->pais);
                $annofundacion = urldecode($equipo->annofundacion);
                $conferencia = urldecode($equipo->conferencia);
                $equipoAnterior = urldecode($equipo->nombre);
            }
        ?>
        ...
        <div class="container-fluid">
            <div class="row mt-3 mb-5 d-flex flex-column align-items-center">
                <form method='post' action='modificarEquipo.php?idequipo=<?php echo $idEquipo; ?>' class='w-75' enctype='multipart/form-data'>
                    <input type="hidden" name="modificar" value="<?php echo $idEquipo; ?>">
                    ...
                    <input type='number' name='anno' class='form-control' id='exampleFormControlInput1' min="1940" max="2024" value='<?php echo $annofundacion ?>' required>
                    <label for='exampleFormControlInput1' class='form-label mt-3'>Conferencia:</label>
                    <select class='form-control' name='conferencia'>
                        <option value="Este" <?php echo ($conferencia == 'Este') ? 'selected' : ''; ?>>Conferencia Este</option>
                        <option value="Oeste" <?php echo ($conferencia == 'Oeste') ? 'selected' : ''; ?>>Conferencia Oeste</option>
                    </select>
                    <label for='exampleFormControlInput1' class='form-label mt-3'>Logo del equipo:</label>
                    <input type='file' name='logoEquipo' class='form-control' id='exampleFormControlInput1'>
