...
<html>
    ...
    <body>
        ...
        <h1 class="text-center mb-5 bg-primary text-white p-5">ALTA JUGADOR</h1>
        ...
        <?php 
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['altajugador']) && $_GET['altajugador'] === 'altajugador') {

                $nombre = urlencode($_GET['nombre']);
                $edad = urlencode($_GET['edad']);
                $posicion = urlencode($_GET['posicion']);
                $nacionalidad = urlencode($_GET['pais']);
                $disponible = urlencode($_GET['disponible']);
                $idequipo = urlencode($_GET['equipo']);

                $url_dame_crearjugador = 'http://iescristobaldemonroy.duckdns.org:81/USER10/NBA_Microservicios/services/crearJugadores.php?nombre='. $nombre . '&edad=' . $edad . '&posicion=' . $posicion . '&nacionalidad=' .$nacionalidad. '&disponible=' .$disponible. '&idequipo='. $idequipo;

                // Hacer la solicitud HTTP y obtener el XML como una cadena
                $xmlString = file_get_contents($url_dame_crearjugador);
        ...
        <div class="container-fluid">
            <div class="row mt-3 mb-5 d-flex flex-column align-items-center">
                <form method='get' action='altaJugador.php' class='w-75' enctype='multipart/form-data'>
                    <input type="hidden" name="altajugador" value="altajugador">
                    ...
                    <input type='number' name='edad' class='form-control' id='exampleFormControlInput1' min="18" max="50" placeholder='Ingrese la edad...' required>
                    ...
                    <div id="equipoForm" style="display: none;">
