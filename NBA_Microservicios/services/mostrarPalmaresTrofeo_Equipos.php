...
try {
    if (count($_GET)===1) {
        ...
        if($paramOk){
            ...
            $sql = $conn->prepare("SELECT e.nombre as nombre, tj.cantidad as cantidad, tj.annos as annos FROM equipos e
            INNER JOIN trofeos_equipos tj ON e.idequipo = tj.idequipo 
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    $courseXML = $responseXML->addChild('equiposGanadores');
